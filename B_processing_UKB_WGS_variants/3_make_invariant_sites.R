...
ukb_invariant_df = remove_observed_sites(ukb_invariant_df, chr, ukb_df)
...
# 10. Annotate predicted positive/negative/null
...
# 11. Merge with other genic features [hs, length, OMIM, etc.]
ukb_invariant_df = merge(ukb_invariant_df, df_gene_info, by='gene_name')

write.csv(ukb_invariant_df, str_interp('./processed/UKB_invariant_variants_annotated_top40hs_AR_chr${chr}.csv'),
          row.names=FALSE)

write.csv(summary_df, str_interp('./processed/filtering_invariant_sumstats_top40hs_AR_chr${chr}.csv'),
          row.names=FALSE)
