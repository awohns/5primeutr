...
# Load data
ukb_df = data.frame(fread('./processed/ukb_180_annotated_processed.csv'))
invar_df = data.frame(fread('./processed/invar_180_annotated_processed.csv'))
ref_df = data.frame(fread('./processed/ref_180_annotated_processed.csv'))

# Process data
ukb_df = ukb_df %>%
  rename(seq_len = 'alt_seq_len_180bp_adjusted') %>%
  select(-alt_seq_len_250bp_adjusted)
...
ref_df = ref_df %>%
  rename(seq_len = 'ref_seq_len_180bp_adjusted') %>%
...
# SAVE 750K PANEL  #
#------------------#
write.csv(final_panel_df, './processed/panel_750k.csv', row.names=FALSE)
write.csv(summary_df, './processed/panel_750k_sumstats.csv', row.names=FALSE)
