...
annotate_pos_neg_null = function(df_annotated){
...
# LOAD GENES  #
#-------------#
genes_raw_df = read.csv('preliminary_dataset_gene_names.csv', header=FALSE)
names(genes_raw_df) = "gene_id"
genes_raw = genes_raw_df$gene_id

# Keep only the genes in this chromosome
genes = genes_raw[genes_raw %in% gencode_df$gene_name]

df_ann_raw = data.frame(fread('./processed/ann_gencode_v45_protein_coding_MANE_clean.csv'))

print("Yep, I loaded everything!")


#---------------------------#
# SIMULATE INVARIANT SITES  #
#---------------------------#
print("I'm entering simulation of invariant sites...")
ukb_invariant_df = simulate_invariant_sites(genes, ref_5utrs_df, gencode_df, limit, alleles)
n_before = nrow(ukb_invariant_df)

ukb_invariant_df$CHR = chr
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_0.csv"))

print("I'm done with that bit!")

#--------------------#
# ANNOTATE VARIANTS  #
#--------------------#
print("I'm entering gene annotation...")
...
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_1.csv"))
...
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_2.csv"))
...
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_3.csv"))
...
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_4.csv"))
...
ukb_invariant_df = remove_vars_further_than_limit(ukb_invariant_df, limit)

summary_df[nrow(summary_df)+1, ] = c("Variants further than 250bp sequence", n_before, nrow(ukb_invariant_df))
n_before = nrow(ukb_invariant_df)

print("Done!")
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_5.csv"))
...
print("Done!")
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_6.csv"))

# 7. Annotate ORFs and Kozak strength in alt sequence
for (i in 1:nrow(ukb_invariant_df)){

  ALT_sequence_250bp = ukb_invariant_df[i, 'ALT_sequence_250bp']
  start_codon = ukb_invariant_df[i, 'start_codon']
  ref_sequence = ukb_invariant_df[i, 'UTR5_sequence_250']

  ukb_invariant_df[i,'orf_ann_250bp'] = classify_orfs(ref_sequence, start_codon)
  ukb_invariant_df[i,'orf_ann_250bp_ALT'] = classify_orfs_ALT(ALT_sequence_250bp, ref_sequence, start_codon)
  ukb_invariant_df[i,'kozak_strength_ALT'] = determine_kozak_strength(ALT_sequence_250bp, start_codon)

}

print("Done!")
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_7.csv"))

# 8. Annotate variant types
ukb_invariant_df = annotate_variant_types(ukb_invariant_df, orf_types,
                                          'orf_ann_250bp', 'orf_ann_250bp_ALT')

ukb_invariant_df = classify_kozak_variants(ukb_invariant_df, 'kozak_strength', 'kozak_strength_ALT')

print("Done!")
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_8.csv"))


# 9. Annotate predicted positive/negative/null
ukb_invariant_df = annotate_pos_neg_null(ukb_invariant_df)

print("Done!")
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_9.csv"))
...
ukb_invariant_df = merge(ukb_invariant_df, df_gene_info, by='gene_name')

print("Done! Saving files now...")
write.csv(ukb_invariant_df, str_interp("./processed/tmp_chr${chr}_10.csv"))

write.csv(ukb_invariant_df, str_interp('./processed/UKB_invariant_variants_annotated_preliminary_panel_chr${chr}.csv'),
          row.names=FALSE)

write.csv(summary_df, str_interp('./processed/filtering_invariant_sumstats_preliminary_panel_chr${chr}.csv'),
          row.names=FALSE)
