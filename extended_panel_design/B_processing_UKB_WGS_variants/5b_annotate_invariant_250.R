...
# INVARIANT data ------------------------------------------------
raw_invar_df = data.frame(
  fread(str_interp("./processed/UKB_invariant_chr${chr}.csv")))

invar_df = process_data(raw_invar_df)
...
#-----------------------#
invar_250_df = merge(invar_250_df, 
                     ref_250_df %>% select(gene_name, REF_sequence_250bp) %>%
                       rename(REF_sequence = 'REF_sequence_250bp'))

invar_250_df$ref_len = nchar(invar_250_df$REF_sequence)
invar_250_df$alt_len = nchar(invar_250_df$ALT_sequence)


# Annotate SEQUENCES
for (i in 1:nrow(invar_250_df)){
  
  ref_sequence = invar_250_df[i, 'REF_sequence']
  alt_sequence = invar_250_df[i, 'ALT_sequence']
  start_codon = invar_250_df[i, 'start_codon']
  
  invar_250_df[i,'main_orf_position'] = determine_main_orf_position(ref_sequence, start_codon)
  invar_250_df[i,'orf_ann_ref'] = classify_orfs(ref_sequence, start_codon)
  invar_250_df[i,'orf_ann_alt'] = classify_orfs_alt(alt_sequence, ref_sequence, start_codon)
  
}


# Annotate VARIANT TYPES 
invar_250_df = annotate_variant_types(invar_250_df, orf_types,
                                      'orf_ann_ref', 'orf_ann_alt')

invar_250_df = adjust_replacements_in_variant_ann(invar_250_df, replacements_df)


# Annotate PREDICTED CATEGORY
invar_250_df = annotate_pos_neg_null_loss(invar_250_df)


#-------------#
# SAVE DATA   #
#-------------#
write.csv(invar_250_df, 
          str_interp('./processed/invariant_250_annotated_chr${chr}.csv'),
          row.names=FALSE)
