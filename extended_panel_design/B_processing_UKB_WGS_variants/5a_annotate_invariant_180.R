...
#-----------------------#
invar_180_df = merge(invar_180_df, 
                     ref_180_df %>% select(gene_name, REF_sequence_180bp) %>%
                       rename(REF_sequence = 'REF_sequence_180bp'))

invar_180_df$ref_len = nchar(invar_180_df$REF_sequence)
invar_180_df$alt_len = nchar(invar_180_df$ALT_sequence)


# Annotate SEQUENCES
for (i in 1:nrow(invar_180_df)){
  
  print(i)
  
  ref_sequence = invar_180_df[i, 'REF_sequence']
  alt_sequence = invar_180_df[i, 'ALT_sequence']
  start_codon = invar_180_df[i, 'start_codon']
  
  invar_180_df[i,'main_orf_position'] = determine_main_orf_position(ref_sequence, start_codon)
  invar_180_df[i,'orf_ann_ref'] = classify_orfs(ref_sequence, start_codon)
  invar_180_df[i,'orf_ann_alt'] = classify_orfs_alt(alt_sequence, ref_sequence, start_codon)
  
}


# Annotate VARIANT TYPES 
invar_180_df = annotate_variant_types(invar_180_df, orf_types,
                                      'orf_ann_ref', 'orf_ann_alt')

invar_180_df = adjust_replacements_in_variant_ann(invar_180_df, replacements_df)


# Annotate PREDICTED CATEGORY
invar_180_df = annotate_pos_neg_null_loss(invar_180_df)


#-------------#
# SAVE DATA   #
#-------------#
write.csv(invar_180_df, 
          str_interp('./processed/invariant_180_annotated_chr${chr}.csv'),
          row.names=FALSE)
