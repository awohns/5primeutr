...
for (i in c(1:22, 'X')){
  ...
  df = fread(str_interp("./processed/UKB_variants_annotated_chr${i}.csv"))
  
  df = df %>% 
    mutate(AF = as.numeric(AC)/as.numeric(AN)) %>%
    mutate(seq_len = nchar(UTR5_sequence_250)) %>%
    select(gene_name:AN, AF, 
           ALT_sequence_250bp, position_within_reporter_seq, 
           start_codon, seq_len,
...
write.csv(ukb_df, "./processed/UKB_observed_variants_annotated.csv",
          row.names=FALSE)
...
write.csv(sumstats, "./processed/filtering_sumstats_all.csv",
          row.names=FALSE)
...
for (i in c(1:22, 'X')){
  ...
  file_path =  str_interp("./processed/UKB_invariant_variants_annotated_top40hs_AR_chr${i}.csv")
  ...
  if (file.exists(file_path)){
    ...
    df = df %>% 
      ...
      select(gene_name, CHR, POS, REF, ALT, seq_len,
             ...
             start_codon, ALT_sequence_250bp,
             orf_ann_250bp, orf_ann_250bp_ALT, 
             kozak_strength, kozak_strength_ALT, 
             variant_type, gained_main_ORF:kozak_stronger, 
             predicted_category, 
...
write.csv(invar_df, "./processed/UKB_invariant_variants_annotated_top40hs_AR.csv",
          row.names=FALSE)
...
for (i in c(1:22, 'X')){
  ...
  file_path =  str_interp("./processed/filtering_invariant_sumstats_top40hs_AR_chr${i}.csv")
...
write.csv(sumstats, "./processed/filtering_sumstats_all_top40hs_AR.csv",
          row.names=FALSE)